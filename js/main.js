/* GMAX NOTÍCIAS — Main JS (design urbano magazine) */
(function() {
    'use strict';
    ...
    function initTheme() {
        ...
        var saved = localStorage.getItem('gmax-theme');
        if (saved) document.documentElement.setAttribute('data-theme', saved);
        if (btn) btn.addEventListener('click', function() {
            var t = document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark';
            document.documentElement.setAttribute('data-theme', t);
            localStorage.setItem('gmax-theme', t);
        });
    }

    function initHero() {
        var el = document.getElementById('hero-bento');
        ...
        el.innerHTML = top.slice(0, 3).map(function(n) {
            var ci = cats[n.categoria] || {nome: n.categoria};
            return '<div class="bento-card" data-id="' + n.id + '">' +
                '<img src="' + n.imagem + '" alt="" loading="lazy">' +
                '<div class="bento-overlay">' +
                '<span class="bento-badge">' + esc(ci.nome || n.categoria) + '</span>' +
                '<div class="bento-title">' + esc(n.titulo) + '</div>' +
                '<div class="bento-meta">' + esc(n.autor) + ' &bull; ' + n.tempo + '</div>' +
                '</div></div>';
        }).join('');
        el.querySelectorAll('.bento-card').forEach(function(c) {
    ...
    function initWeather() {
        ...
        el.innerHTML = '<h3 class="widget-title">Clima</h3>' +
            ...
            '<div class="w-desc">' + esc(c.condicao||'') + ' em ' + esc(c.cidade||'São Paulo') + '</div>' +
            '<div class="w-details">' +
    ...
    function initShare() {
        ...
        document.querySelectorAll('.share-item').forEach(function(b) {
            b.addEventListener('click', function() {
                var p = this.getAttribute('data-p'), t = ov.getAttribute('data-title')||'GMAX NOTÍCIAS', u = location.href;
