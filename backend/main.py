"""
GMAX NOTÍCIAS — Orquestrador Principal
...
logger = logging.getLogger('gmaxnoticias')
...
def _git_push_automatico():
    try:
        ...
        subprocess.run(['git', 'commit', '-m', f'Atualização automática GMAX NOTÍCIAS - {agora}'], cwd=repo_dir, check=True, capture_output=True)
...
def configurar_logging():
    ...
    log_file = os.path.join(CAMINHO_LOG, f'gmaxnoticias_{datetime.now().strftime("%Y%m%d")}.log')
    logging.basicConfig(
        level=logging.INFO,
...
def executar_atualizacao():
    inicio = datetime.now()
    logger.info("=" * 60)
    logger.info("GMAX NOTÍCIAS — Iniciando atualização")
    logger.info(f"Data/hora: {inicio.strftime('%d/%m/%Y %H:%M:%S')}")
    logger.info("=" * 60)

    try:
        logger.info("\n ETAPA 1: Coletando notícias...")
        from coletor_sp import coletar_todas_sp
        noticias_sp = coletar_todas_sp()
        logger.info(f"  São Paulo: {len(noticias_sp)} notícias")
        ...
        todas = noticias_sp + noticias_externas
        ...
        if GEMINI_API_KEY:
            ...
            trending = ['#GMAXNoticias', '#SãoPaulo', '#Brasil', '#Economia', '#Política', '#Esportes']
